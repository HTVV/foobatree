...
async function openChildPopup() {
    ...
    for (var i = 0; i < person.spouses.length; i++) {
        document.getElementById('otherParentSelect').innerHTML += `<option value="${person.spouses[i]}">${await idToName(person.spouses[i])}</option>`
    }
...
function openDetailsPopup(typeEdit) {
    ...
    if (typeEdit == "birth") {
        ...
        <p>Birth date</p>
        <input class="popup-input" type="date" placeholder="" id="BirthDateInput" value="${person.birthDate}">
        <p>Birth place</p>
        <input class="popup-input" type="text" placeholder="" id="BirthPlaceInput" value="${person.birthPlace}">
        <p>Gender</p>
        <input class="popup-input" type="text" placeholder="" id="GenderInput" value="${person.gender}">
        <br></br>
        <div>
            <input type="radio" value="alive" name="changeStatus" ${person.status == "alive" ? "checked" : ""}>Living</input>
    ...
    if (typeEdit == "name") {
        ...
        <p>First name(s)</p>
        <input class="popup-input" type="text" id="FirstNameInput" value="${person.firstName}">
        <p>Last name(s)</p>
        <input class="popup-input" type="text" id="LastNameInput" value="${person.lastName}">
        <p>Birth last name(s)</p>
        <input class="popup-input" type="text" id="OgNameInput" value="${person.ogName}">
        <p>Patronym</p>
        <input class="popup-input" type="text" id="PatronymInput" value="${person.patronym}">
        <p>Lore</p>
        <input class="popup-input" type="text" id="LoreInput" value="${person.lore ? person.lore : ""}">
        <p>Link to profile picture</p>
        <input class="popup-input" type="text" id="img-input" value="${person.pic ? person.pic : ""}">
        <br></br>
        <img id="image-preview" style="width: 300px; max-height: 300px;">
        `

        if (person.pic) document.getElementById("image-preview").src = document.getElementById("img-input").value;
...
function textPopup(typeEdit) {
    ...
    if (typeEdit == "writing") {
        ...
        document.getElementById("WritingInput").value = person.writing.replaceAll("%79", "+").replaceAll("%89", "&")
    }
    if (typeEdit == "sources") {
        document.getElementById("popup3Content").innerHTML = `
        <h3>Edit sources</h3>
        ...
        `
        document.getElementById("SourcesInput").value = person.sources.replaceAll("%79", "+").replaceAll("%89", "&")
    }
}
...
async function submitForm() {
    if (type == "parents") {
        ...
        if (person.parent1Id == null || person.parent1Id == "") {
            ...
            const oldParent = await (await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${person.parent1Id}${requestEnd}`)).json()
            oldParent[0].spouses.push(newUuid)
            parent.spouses.push(person.parent1Id)
            fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${person.parent1Id}&content=${encodeURI(JSON.stringify(oldParent[0]))}${requestEnd}`)
        }
        ...
        //child
        fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${uuid}&content=${encodeURI(JSON.stringify(person))}${requestEnd}`)
    ...
    if (type == "details") {
        ...
        person.birthDate = document.getElementById("BirthDateInput") ? document.getElementById("BirthDateInput").value : person.birthDate
        person.birthPlace = document.getElementById("BirthPlaceInput") ? document.getElementById("BirthPlaceInput").value : person.birthPlace
        person.deathDate = document.getElementById("DeathDateInput") ? document.getElementById("DeathDateInput").value : person.deathDate
        person.deathPlace = document.getElementById("DeathPlaceInput") ? document.getElementById("DeathPlaceInput").value : person.deathPlace
        person.causeOfDeath = document.getElementById("DeathCauseInput") ? document.getElementById("DeathCauseInput").value : person.causeOfDeath
        person.burialPlace = document.getElementById("BurialPlaceInput") ? document.getElementById("BurialPlaceInput").value : person.burialPlace
        person.firstName = document.getElementById("FirstNameInput") ? document.getElementById("FirstNameInput").value : person.firstName
        person.lastName = document.getElementById("LastNameInput") ? document.getElementById("LastNameInput").value : person.lastName
        person.ogName = document.getElementById("OgNameInput") ? document.getElementById("OgNameInput").value : person.ogName
        person.patronym = document.getElementById("PatronymInput") ? document.getElementById("PatronymInput").value : person.patronym
        person.lore = document.getElementById("LoreInput") ? document.getElementById("LoreInput").value : person.lore
        person.name = btrim(`${person.firstName} ${person.patronym} ${person.lastName}${person.ogName != "" ? ` (${person.ogName})` : ""}`)
        person.gender = document.getElementById("GenderInput") ? document.getElementById("GenderInput").value : person.gender;
        person.pic = document.getElementById("img-input") ? document.getElementById("img-input").value : person.pic;
        person.status = document.querySelector('input[name="changeStatus"]') ? document.querySelector('input[name="changeStatus"]:checked').value : person.status
        console.log(person)
        fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${uuid}&content=${encodeURI(JSON.stringify(person))}${requestEnd}`)
    }
    if (type == "text") {
        console.log("YES")
        person.writing = (document.getElementById("WritingInput") ? document.getElementById("WritingInput").value : person.writing).replaceAll("+", "%79").replaceAll("&", "%89")
        person.sources = (document.getElementById("SourcesInput") ? document.getElementById("SourcesInput").value : person.sources).replaceAll("+", "%79").replaceAll("&", "%89")
        fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${uuid}&content=${encodeURI(JSON.stringify(person))}${requestEnd}`)
    }
    if (type == "spouse") {
        if (!document.querySelector('input[name="maleFemale"]:checked')) {
    ...
    if (type == "child") {
        ...
        if (parent2) {
            ...
            fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${parent2}&content=${encodeURI(JSON.stringify(parent2Data))}${requestEnd}`)
        }
        //chiöd
...
async function main() {
    ...
    person = await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${uuid}${requestEnd}`)
    ...
    document.getElementById("children-relation-header").textContent += ` (${person.children.length})`
    ...
    document.getElementById("writingText").textContent = person.writing.replaceAll("%79", "+").replaceAll("%89", "&")
    document.getElementById("sourcesText").textContent = person.sources.replaceAll("%79", "+").replaceAll("%89", "&")
    document.getElementById("gender").textContent = person.gender.charAt(0).toUpperCase() + person.gender.slice(1)
...
async function relativeCheck() {
    ...
    if (person.parent1Id) {
        parentTest = await (await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${person.parent1Id}${requestEnd}`)).json()
    ...
    if (person.parent2Id) {
        parentTest = await (await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${person.parent2Id}${requestEnd}`)).json()
        if (parentTest.error == 400) {
    ...
    for (var i = 0; i < person.children.length; i++) {
        childTest = await (await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${person.children[i]}${requestEnd}`)).json()
    ...
    if (todelete.length > 0) {
        ...
        for (var i = todelete.length; i > 0; i--) {
            if (person.children.length != 1) {
                person.children = person.children.splice(person.children.indexOf(todelete[i]), 1);
            } else {
                person.children = []
            }
        }
    ...
    todelete = [];
    for (var i = 0; i < person.spouses.length; i++) {
        spouseTest = await (await fetch(`https://familytree.loophole.site/getProfile?token=${token}&profileUuid=${person.spouses[i]}${requestEnd}`)).json()
    ...
    if (todelete.length > 0) {
        ...
        for (var i = todelete.length; i > 0; i--) {
            if (person.spouses.length != 1) {
                person.spouses = person.spouses.splice(person.spouses.indexOf(todelete[i]), 1);
            } else {
                person.spouses = []
            }
        }
...
async function showRelatives() {
    ...
    if (person.parent1Id && person.parent2Id) {
        document.getElementById("parents-container").innerHTML += personToRelativeLabel(idToData(person.parent1Id))
        document.getElementById("parents-container").innerHTML += personToRelativeLabel(idToData(person.parent2Id))
    }
    ...
    //spouses
    for (var i = 0; i < person.spouses.length; i++) {
        document.getElementById("spouses-container").innerHTML += personToRelativeLabel(idToData(person.spouses[i]))
    }
...
async function deletePerson() {
    if (person.parent1Id) {
        ...
        await fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${parent1.id}&content=${encodeURI(JSON.stringify(parent1))}${requestEnd}`)
    }
    if (person.parent2Id) {
        const parent2 = idToData(person.parent2Id)
        const index = parent2.children.indexOf(person.id)
        parent2.children.splice(index, 1);
        await fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${parent2.id}&content=${encodeURI(JSON.stringify(parent2))}${requestEnd}`)
    }
    if (person.children.length != 0) {
        person.children.forEach(async element => {
            ...
            await fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${child.id}&content=${encodeURI(JSON.stringify(child))}${requestEnd}`)
        });
    }
    if (person.spouses.length != 0) {
        person.spouses.forEach(async element => {
            const spouse = idToData(element)
            spouse.spouses = spouse.spouses.splice(spouse.spouses.indexOf(person.id), 1);
            await fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${spouse.id}&content=${encodeURI(JSON.stringify(spouse))}${requestEnd}`)
        });
    }
    await fetch(`https://familytree.loophole.site/setProfile?token=${token}&profileUuid=${person.id}&content=${encodeURI(JSON.stringify())}${requestEnd}`)
    window.close()
}
