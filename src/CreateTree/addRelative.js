  ...
  function onChange(updated_datum, props) {
    ...
    function updateGendersForNewRelatives() {
      ...
      data.forEach(d => {
        ...
        if (!rd) return
        if (rd.rel_type === 'spouse') d.data.gender = d.data.gender === 'male' ? 'female' : 'male'
...
function addDatumRelsPlaceholders(datum, store_data, addRelLabels) {
  if (!datum.rels.father) {
    const father = createNewPerson({data: {gender: "male"}, rels: {children: [datum.id]}})
  ...
  if (!datum.rels.mother) {
    const mother = createNewPerson({data: {gender: "female"}, rels: {children: [datum.id]}})
  ...
  if (datum.rels.children) {
    ...
    datum.rels.children.forEach(child_id => {
      ...
      if (!child.rels.mother) {
        if (!new_spouse) new_spouse = createNewPerson({data: {gender: "female"}, rels: {spouses: [datum.id], children: []}})
      ...
      if (!child.rels.father) {
        if (!new_spouse) new_spouse = createNewPerson({data: {gender: "male"}, rels: {spouses: [datum.id], children: []}})
        new_spouse._new_rel_data = {rel_type: "spouse", label: addRelLabels.spouse, rel_id: datum.id}
        new_spouse.rels.children.push(child.id)
        datum.rels.spouses.push(new_spouse.id)
  ...
  const spouse_gender = datum.data.gender === "male" ? "female" : "male"
  ...
  datum.rels.spouses.forEach(spouse_id => {
    ...
    const mother_id = datum.data.gender === "male" ? spouse.id : datum.id
    const father_id = datum.data.gender === "female" ? spouse.id : datum.id
    if (!spouse.rels.children) spouse.rels.children = []
    
    const new_son = createNewPerson({data: {gender: "male"}, rels: {father: father_id, mother: mother_id}})
    ...
    const new_daughter = createNewPerson({data: {gender: "female"}, rels: {mother: mother_id, father: father_id}})
