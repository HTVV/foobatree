...
export function createForm({
  ...
  const children_added = (datum.rels.children || []).some((c_id) => {
  ...
  form_creator.gender_field = {
    ...
    disabled:
      ["father", "mother"].some(
        (rel) => rel === datum._new_rel_data?.rel_type
      ) || children_added,
    options: [
  ...
  function addSelectField(field) {
    if (!field.optionCreator && !field.options)
      return console.error(
        "optionCreator or options is not set for field",
        field
      );
    form_creator.fields.push({
      id: field.id,
