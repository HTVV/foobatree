...
RemoveRelative.prototype.activate = function(datum) {
  ...
  function onChange(rel_tree_datum, onAccept) {
    ...
    function handleSpouseRemoval() {
      ...
      function openModal() {
        const current_gender_class = datum.data.gender === 'male' ? 'f3-male-bg' : datum.data.gender === 'female' ? 'f3-female-bg' : null
        const spouse_gender_class = spouse.data.gender === 'male' ? 'f3-male-bg' : spouse.data.gender === 'female' ? 'f3-female-bg' : null
