...
export default function linkSpouseText(svg, tree, props={}) {
  const links_data = []
  tree.data.forEach(d => {
    if (d._spouse && d.data.data.gender === 'female') links_data.push({nodes: [d, d._spouse], id: `${d.data.id}--${d._spouse.data.id}`})
  ...
  const spouseLineX = (sp1, sp2) => {
    if (sp1.spouse && sp1.data.data.gender === 'female') return sp1.x - props.node_separation/2
    else if (sp2.spouse && sp2.data.data.gender === 'male') return sp2.x + props.node_separation/2
